...
class CategoryController {
              ...
              .then(function (data) {
                if (type_route != 1 && typeof type_route !== "undefined") {
                  res.render("page/category/grid", {
                    ...
                    data: data,
                    edit: "/danh-muc-san-pham/sua",
                    destroy: "/danh-muc-san-pham/xoa",
                    create: "/danh-muc-san-pham/them",
                    export_pdf: "/danh-muc-san-pham/export-pdf",
                    export_excel: "/danh-muc-san-pham/export-excel",
                    export_word: "/danh-muc-san-pham/export-word",
                    ...
                    perPage: perPage,
                    page: page,
                    destroy_multi: "/danh-muc-san-pham/xoa-multi",
                  });
                }
                if (
                  ...
                  if (search != "") {
                    res.render("page/category/list-item", {
                      field: [
                        {
                          value: "name",
                          title: "Tên danh mục",
  ...
  async post(req, res, next) {
    const data = await CategoryModel.findOne().sort({ number: -1 });
    const name = req.body.name;

    var code = "";
    if (data !== null) {
      code = func_commom.gennateCodeProduct("DM", data.count + 1);
      CategoryModel.create({
        ...
        .then(function () {
          return res.redirect("/danh-muc-san-pham/danh-sach");
        })
        .catch();
    } else {
      code = func_commom.gennateCodeProduct("DM", 1);
      CategoryModel.create({
        name: name,
        code: code,
        count: 1,
      ...
      .then(function (data) {
        if (data) {
          res.render("page/supplier/edit", {
            ...
            heading: "Sửa",
            module: "Danh mục sản phẩm",
            action: "/danh-muc-san-pham/sua/" + id,
            input: [
              {
                id: "name",
                placeholder: "Tên Danh mục sản phẩm",
  ...
  delete_multi(req, res, next) {
    CategoryModel.deleteMany({ _id: req.body.data })
      .then(function (data) {
        if (data) {
          res.status(200).json({ status: 200 });
          //   return res.redirect("/danh-muc-san-pham/danh-sach");
